 ...
 *
 * DON'T confuse this prompt's "block A" output with the FieldMap type in
 * complaint.ts — they're two different things that happen to cover
 * similar ground:
 *   - FieldMap (complaint.ts/buildFieldMap) is computed in plain
 *     TypeScript, sent to the client immediately as the `fields` NDJSON
 *     event, and used for a quick "here's what we're using" preview.
 *   - Block A below is generated BY THE MODEL, streamed in as part of
 *     `text`, and is what actually ends up in the rendered PDF/companion
 *     doc. It's allowed to read richer context (causes, security
 *     deposit, court history) that FieldMap doesn't bother summarizing.
 */
